...
Write a `.proto`, compile it with prost-build, and attach the `#[smsg]` macro to the
module. The macro reads the descriptor set at compile time and generates
`EnvelopeMeta` for every message in the matching package:

```rust
#[smsg("descriptors.pb")] // generated by prost-build / protoc --descriptor_set_out
pub mod chat {
